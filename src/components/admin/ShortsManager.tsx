...
import { Plus, Trash2, Edit2, Loader2, Film, Link2 } from "lucide-react";
...
export default function ShortsManager() {
  ...
  const resetForm = () => {
    ...
    setEditingId(null); setShowForm(false); setUploadProgress(""); setUploadPercent(0); setUploadDetail(null);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (videoFile) {
        ...
        setUploadPercent(0);
        videoUrl = await uploadVideoToR2(videoFile, "shorts", (p) => setUploadPercent(p), (d) => setUploadDetail(d));
      }
      if (thumbnailFile) {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (error) {
      ...
      if (short) deleteR2Files([short.video_url, short.thumbnail_url]);
